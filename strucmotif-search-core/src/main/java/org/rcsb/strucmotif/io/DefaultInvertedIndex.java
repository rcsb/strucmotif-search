...
public class DefaultInvertedIndex implements InvertedIndex {
    ...
    public ArrayBucket select(int residuePairDescriptor) {
        ...
        try {
            ...
            handleReadError(residuePairDescriptor, filename, e); // should throw or exit
            return null; // unreachable
        }
    }

    /**
     * Handle a failed read, which usually indicates that the file bundle entered a bad state, which isn't easily
     * recoverable but rather leads to subsequent accesses failing too.
     ...
     */
    private void handleReadError(int residuePairDescriptor, String filename, IOException e) {
